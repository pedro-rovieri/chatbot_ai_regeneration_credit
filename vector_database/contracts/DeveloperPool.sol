 ...
 * @title DeveloperPool
 * @author Sintrop
 * @notice This contract manages the distribution of Regeneration Credit tokens as rewards to developers
...
contract DeveloperPool is Poolable, Blockable, Callable, ReentrancyGuard {
   ...
   */
  function setContractCall(address _developerRulesAddress) external onlyOwner {
   ...
   * @notice This function can only be called by the DeveloperRules contract, whitelisted via the `Callable` contract's mechanisms.
  ...
  /**
   * @dev Allows an authorized caller to decrease an developer's pool level.
