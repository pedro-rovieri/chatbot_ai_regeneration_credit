...
This contract manages the distribution of Regeneration Credit tokens as rewards to inspectors
...
### inspectorRulesAddress

```solidity
address inspectorRulesAddress
```

The address of the `InspectorRules` contract.
...
_Initializes the InspectorPool contract.
...
```solidity
function setContractCall(address _inspectorRulesAddress) external
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _inspectorRulesAddress | address | Address of InspectorRules. |
...
This function can only be called by the InspectorRules contract, whitelisted via the `Callable` contract's mechanisms.
The user must also be eligible for withdrawal based on the `Blockable` contract's era tracking.

_Allows an authorized caller, the Inspector contract, to trigger a token withdrawal for a user.
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| delegate | address | The address of the user (inspector) for whom the withdrawal is being processed. |
...
This function updates the inspector level within the system's pooling mechanism._
...
_Allows an authorized caller to decrease an inspector's pool level.
This function adjusts the inspector's level downwards within the system's pooling mechanism._
